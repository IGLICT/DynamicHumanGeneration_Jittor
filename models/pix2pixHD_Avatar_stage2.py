...
class Pix2PixHD_Avatar(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.isTrain:
            ...
            tmp_tensor = torch.zeros_like(texture_tensor).repeat(1,4,1,1) # (1, 24x3x4, tex_size, tex_size)
            # tmp_tensor = torch.zeros_like(texture_tensor).repeat(1,2,1,1) # (1, 24x3x2, tex_size, tex_size)
        ...
        if TEST:
            ...
            tmp_tensor = torch.zeros_like(texture_tensor).repeat(1,4,1,1) # (1, 24x3x5, tex_size, tex_size)
            # tmp_tensor = torch.zeros_like(texture_tensor).repeat(1,2,1,1) # (1, 24x3x2, tex_size, tex_size)
            texture_tensor = util.catTextureTensor(texture_tensor, tmp_tensor)
            self.texture = torch.nn.Parameter(texture_tensor.to(device=self.device))
    ...
    def init_optimizer_G(self, epoch, StaticEpoch=3):
        ...
        if epoch <= StaticEpoch:
            ...
            ratio = decay_rate ** (epoch)
            self.optimizer_G = torch.optim.Adam([{'params': self.TransG.parameters(), 'lr': self.lr},
                                                 {'params': self.texture, 'lr': self.lr},
                                                 {'params': self.Feature2RGB.parameters(), 'lr': self.lr},
                                                 {'params': self.BG, 'lr': self.lr}], betas=(self.beta1, 0.999))
        else:
            ratio = decay_rate ** (epoch)
            print("The epoch is %d, decrease TransG with rate %s!" % (epoch, ratio))
            # update All
    ...
    def forward(self, epoch, texture, Pose, mask, real, real_uv, real_cls, bg, \
        ...
        fg_image = util.texture2image(gen_texture, UVs.detach(), Probs.detach())
        fg_image_raw = util.texture2image(gen_texture, UVs, Probs, selNUM=3)
        fg_image_raw = fg_image_raw*2 - 1

        # feature_input = torch.cat([TransG_input], dim=1)
        feature_input = torch.cat([TransG_input, fg_image], dim=1)
        # feature_input = torch.cat([TransG_input], dim=1)
        fg_image = self.Feature2RGB(feature_input)
        ...
        fake_image = fg_image * (1-bg_mask.detach()) + bg_image * (bg_mask.detach())
        # fake_image = fg_image
    ...
    ### VGG loss ###
        ...
        if not self.opt.no_vgg_loss:
            ...
            if epoch > StaticEpoch:
                loss_G_VGG += self.criterionVGG(fake_image, real_images) * self.opt.lambda_feat / 5
                # loss_G_VGG *= 0.5
                ...
                for i in range(self.opt.num_D):
                    for j in range(len(pred_fake[i])-1):
                        loss_G_VGG += D_weights * feat_weights * \
                            self.criterionFeat(pred_fake[i][j], pred_real[i][j].detach()) * self.opt.lambda_feat / 5

    ### L2 loss ###
        loss_L2_mask = 0
        loss_L2_mask += self.criterionL2_mask(fake_image_raw, real_images) * self.opt.lambda_L2 / 20
        if epoch > StaticEpoch:
            loss_L2_mask += self.criterionL2_mask(fake_image, real_images) * self.opt.lambda_L2 / 20
            # loss_L2_mask *= 0.5
    ...
    ### classify loss ###
        loss_Prob = self.criterion_Prob(Probs, real_clss.long()) * self.opt.lambda_Prob / 10
    ### mask loss ###
        loss_G_mask = 0
