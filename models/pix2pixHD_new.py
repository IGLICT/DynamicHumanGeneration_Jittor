...
class Pix2PixHD_Avatar(BaseModel):
    ...
    def initialize(self, opt):
        ...
        ### add opt
        opt.face_discrim = None
        opt.face_generator = None
        opt.use_l1 = False
        opt.niter_fix_main = 0
        opt.label_nc = 6
        ...
        if not opt.no_instance:
            netG_input_nc += 1          
        self.netG = networks.define_G(netG_input_nc, opt.output_nc, opt.ngf_global, opt.netG, 
    ...
    def forward(self, epoch, texture, label, mask, image, real_uv, real_cls, bg, \
        next_label, mask_before, next_image, real_uv_before, real_cls_before, flow, flow_inv, infer=False):
        # Encode Inputs
        ...
        zeroshere = torch.zeros_like(input_label)
        face_coords = torch.zeros(4,4)
        ...
        if self.opt.face_generator:
            ...
            print(maxx.data,minx.data,maxy.data,miny.data)
        ...
        fake_image_bf = warped_image = warped_real_image = warped_image_comp = conf = torch.zeros_like(I_0)
        fg_image = fg_image_raw = bg_image = bg_mask = mask_tex = fake_image_bf
        fake_image = I_0
        UVs = torch.zeros_like(conf[:,:1]).expand(-1,48,-1,-1)
        Probs = torch.zeros_like(conf[:,:1]).expand(-1,25,-1,-1)
        gen_texture = torch.zeros(1,72,100,100).to(fake_image.device)
        # self.loss_names = ['G_GAN', 'G_GAN_Feat', 'G_VGG', 'D_real', 'D_fake', 'G_GANface', 'D_realface', 'D_fakeface']
        return [[loss_G_GAN, 0, loss_G_VGG+loss_G_GAN_Feat, loss_D_real, loss_D_fake, 0, 0, 0], \
